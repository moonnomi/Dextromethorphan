...
|---|---|
| Albums, Artists, Genres | Cards in the active gallery; image decode remains visible-container-only |
...
Changing tabs, changing search results, opening or closing a collection, and restoring Mouse4/Mouse5 navigation cancel the previous scope and create a new active-view scope. Cards that already have artwork are filtered out before work is scheduled. The later gallery correctness hardening exposes the complete lightweight card list immediately; WPF still creates and decodes only visible image controls.
...
- Album work remained isolated to the active Album scope.
