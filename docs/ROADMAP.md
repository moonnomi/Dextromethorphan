...
- [x] **P0 VIEW-001** Replace the eager gallery `WrapPanel` with a responsive row-virtualized grid backed by WPF's built-in `VirtualizingStackPanel`; incremental loading alone is not virtualization. See [the artwork and virtualization result](performance/ARTWORK-VIRTUALIZATION-2026-07-25.md) and [the real-library rendering regression](performance/GALLERY-RENDERING-REGRESSION-2026-07-28.md).
...
- [ ] **P1 HW-004** Run 8-hour playback/transition soak tests and record callback deadline misses, recovery attempts, endpoint-volume invariance, and memory growth. The isolated schema-3 runner, atomic checkpoints, strict report validator, and detached Task Scheduler smoke pass are complete; the real eight-hour report is still required. See [audio soak qualification](audio/AUDIO-SOAK-QUALIFICATION-2026-07-31.md).
