...
The current release test suite passes 281 automated tests. The opt-in onboard-device gate separately passed 36/36 shared formats, 24/24 driver-supported exclusive formats, and 2/10/100 ms buffers with endpoint volume unchanged. The external DST gate separately decoded three consecutive compressed frames bit-for-bit and verified cross-frame DoP seeking without modifying its source fixtures. Generated DSD64/128 tests verify marker order, two- and six-channel interleave, odd callback alignment, and seeking for DSF, uncompressed DFF, and DST-contained DSD. The physical DoP runner additionally generates finite DSD64/128 DSF silence, requires a pinned non-default endpoint and traceable DAC metadata, exercises direct exclusive playback and repeated seeks, and treats missing DAC-display evidence as non-qualifying. It has not been sent to the current onboard endpoint. The schema-3 soak runner atomically checkpoints its report, pins the resolved endpoint, monitors volume throughout, and distinguishes a short run pass from the eight-hour gate. Its detached Task Scheduler smoke completed 20.0297687 observed playing seconds and seven mixed-rate crossfaded transitions with no excluded gaps, non-playing time, callback deadline misses, recovery attempts, or volume changes. This validates the runner and detached host, not the eight-hour acceptance gate.
...
| HW-004: long soak | The isolated schema-3 runner, atomic checkpoints, strict validator, and detached live smoke pass; eight observed playing hours have not yet completed | Retain a schema-3 report accepted by `Test-AudioSoakReport.ps1` |
