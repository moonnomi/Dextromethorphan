...
public sealed class SqliteLibraryRepository(AppPaths paths) : ILibraryRepository
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        var hadSearchIndex = await ObjectExistsAsync(connection, "table", "tracks_fts", cancellationToken);
        await ExecuteScriptAsync(connection, BaseSchema, cancellationToken);
        await EnsureColumnAsync(connection, "tracks", "artwork_path", "TEXT", cancellationToken);
        await ExecuteScriptAsync(connection, SearchSchema, cancellationToken);
        if (!hadSearchIndex)
            await ExecuteScriptAsync(connection, "INSERT INTO tracks_fts(tracks_fts) VALUES('rebuild');", cancellationToken);
    }
    ...
    public async Task RemoveMissingAsync(IReadOnlyCollection<string> roots, CancellationToken cancellationToken = default)
    {
        ...
        delete.CommandText = "DELETE FROM tracks WHERE id = $id";
    ...
    public async Task<LibraryStats> GetStatsAsync(CancellationToken cancellationToken = default)
    {
        await using var connection = await OpenAsync(cancellationToken);
        await using var command = connection.CreateCommand();
        command.CommandText = "SELECT COUNT(*), COUNT(DISTINCT NULLIF(album, '')), COUNT(DISTINCT NULLIF(artist, '')), COALESCE(SUM(duration_ms), 0) FROM tracks";
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
    ...
    private static void AddTrackParameters(SqliteCommand c, Track t)
    {
        ...
        {
            ["$path"] = Path.GetFullPath(t.Path), ["$title"] = t.Title, ["$artist"] = t.Artist, ["$album_artist"] = t.AlbumArtist, ["$album"] = t.Album, ["$genre"] = t.Genre, ["$comment"] = t.Comment,
    ...
    private static async Task ExecuteScriptAsync(SqliteConnection connection, string script, CancellationToken cancellationToken)
    {
        await using var command = connection.CreateCommand();
    ...
    private const string BaseSchema = """
        ...
        CREATE TABLE IF NOT EXISTS tracks(
          ...
          play_count INTEGER NOT NULL DEFAULT 0, last_played_at INTEGER, file_modified_at INTEGER NOT NULL, file_size INTEGER NOT NULL, artwork_path TEXT, lyrics TEXT NOT NULL DEFAULT '', added_at INTEGER NOT NULL, updated_at INTEGER NOT NULL);
